...
def cache_data(
    ...
    path: str,
    timing: str,
    win: tuple[float, float] = (-1.2, 0),  # (-0.5, 0.9), (-1.2, 0)
) -> None:
    """
...
if __name__ == "__main__":
    ...
    # available subjects
    subjects = [s[3:] for s in os.listdir(asc_path) if s.startswith("sub")]
    bad_subjects = [
        "001",  # handled separately
        "002",  # handled separately
        "003",  # handled separately
        "004",  # handled separately
        "005",  # handled separately
        "006",  # handled separately
        "007",  # handled separately
        "037",  # proc file missing
        "108",  # ST missing
        "115",  # ST missing
        "120",  # ST missing
        "121",  # ST missing
    ...
    for s in [f"{i:03d}" for i in range(19, 43)]:
        sr[s] = 250
    ...
    for subject in tqdm(subjects_copy):
        ...
        try:
            sub_data = np.load(
                os.path.join(subject_path, f"{subject}_{task}_tracks.npy")
            )
            markers = np.load(
                os.path.join(subject_path, f"{subject}_{task}_markers.npy")
            )
        ...
        # for each category, prepare aggregate samples
        cat_samples = {
    ...
    # save the trial dictionary as JSON
    dpath = f"/PATH/TO/eyelink-processed/metadata/{task}_{trials_in_set}_{timing}{name}.json"
